...
| X11 | **The savings scan read ONE of the two transcript carriers**, so every session written in the current transcript generation was invisible to it. A turn's usage lands either in a stream chunk (`assistant/chunk` → `chunk.usage`, the legacy `session.jsonl.zstd` format) or on the settled message (`assistant/message` → `data.usage`, the current `session.v3.jsonl.zstd` format), and a file uses exactly one of them. Reading only the chunk carrier priced 9,222 of this machine's 26,198 billed turns: the panel showed ¥3308.52 where the same history reads ¥16,670.78 once every carrier is read and the overlaps of X14/X15 are handled. Cline was the extreme case — **every** Cline session is current-format, so the scan found none of them, and its row survived only because the live hook counts usage off the adapter stream; the next re-scan deleted it. | reproduced (a faithful replay of the shipped scan reproduces the panel's figure to the cent; `temp/_audit/carrier-audit.mjs`, `style-timeline.mjs`) | **adopted** — both carriers are read and merged per `(turn, step)`, chunk first (identical where both exist, verified across every file that has both). A `SCAN_VERSION` gate discards an older document instead of displaying it, because such a document is under-counted, not merely old. |
| X12 | **The rate table priced by vendor substring and modelled three buckets.** `gpt-5.6-luna` matched the `gpt-5` rule — a 12× overcharge — while `cline-pass/mimo-v2.6-pro` matched nothing and silently took the generic default. Cache WRITE tokens were dropped outright (no field on the summary), and the DeepSeek rows carried the *off-peak* rate unconditionally, so every weekday peak-window turn (01–04 and 06–10 UTC) was billed at half price. | measured over this machine's history: 904 of 3,086 Cline DeepSeek turns and 1,514 of 2,472 Command Code DeepSeek turns fall inside peak windows | **adopted** — `src/stats/model-prices.ts` vendors the published table (per-row `cacheWrite`, `peak`, `contextTiers`), resolves per catalog id before falling back to a family rule, marks anything unpublished as approximate, and reports unpriced cache-write tokens rather than inventing a multiplier. |
| X13 | **The panel's "Recalculate" button never recalculated.** It called `tokenStats`, which answered from the cached summary, under a label that promises a fresh walk. | trace | **adopted** — `tokenStats` takes `refresh`, and the button sends it (`getTokenSavingsSummary(force)`). |
| X14 | **One session recorded in BOTH transcript formats was counted twice.** A directory written across the format change keeps the legacy transcript beside the new one, and the two repeat a core of the same requests verbatim. 56 of this machine's 284 session directories are in that state: 349M tokens of overlap, ¥911 of the estimate, billed twice. (An earlier reading of the same evidence blamed a *copied session*; the twins all sit in one directory with the two standard file names, so the migration is the mechanism.) | reproduced (record identity = provider \| model \| all four buckets \| timestamp; `temp/_audit/water-overlap.mjs`, whose extraction reproduces the panel's own total to the cent before it measures the overlap) | **adopted** — the walk deduplicates per DIRECTORY by that identity, never globally, so two identical requests in two different sessions stay two requests. `SCAN_VERSION` 3. |
| X15 | **A context compaction is a billed request the scan never read.** The summarizer call is recorded as `compaction/summary` with its OWN `provider`/`model` and a `usage` object — not as a turn, and not on any stream chunk. 43 such requests in this history, 11.9M tokens, ¥121 across all providers (of which the hub's share is smaller). | trace (`temp/_audit/compaction-probe.mjs`) | **adopted** — the third carrier is read, attributed to the route the event names. |
...
| --- | --- | --- | --- | --- | --- |
| A1 | **Session-cost readout** | The harness's own token pill gains a trailing amount (`1.2M tokens · Cache hit 87% · ≈$0.0123`), and each row of the usage dialog it opens gains a price. Tooltip carries the per-bucket breakdown plus "estimate from published rates, not the provider invoice". | `ref-commandcode-provider/src/client/session-cost-display.ts:249-285` (injects into `[data-composer-stats]` / `[data-session-stats-usage]`; rows matched positionally and confirmed by token count), `session-cost.ts:267-358` (all numbers/copy, React-free), `cost-projection.ts:56-97` (durable fold on the `sessionProjections` seam) | M–L | The dock pill and the dialog portal already exist (`SubscriptionUsageBadge.tsx:271-293,446`), and the **pricing half now exists too** (`src/stats/model-prices.ts`: published rows with peak windows, context tiers and cache-write rates, plus a per-request `priceUsage`). "Missing" from this row is therefore narrowed to the **per-session accumulator** (`recordStreamTokenUsage` has zero call sites) and any `sessionProjections` registration. |
