...
describe('zed paste and catalog', () => {
  ...
  it('reports the bundle for a Student plan from the documented table, not a number derived from its name', () => {
    // I WRONGLY deleted this allowance, then wrongly "fixed" the test to assert its
    // absence. The figure is real: Zed publishes an included-token bundle on its
    // pricing page and its own UI shows one for a Student account. What was actually
    // wrong was the DERIVATION — a flat $10 for every pro-class plan, which is Pro's
    // SUBSCRIPTION PRICE ($10/month) rather than its allowance ($5).
    //
    // The pricing page lists Free, Pro, Business and Trial and never mentions Student,
    // so a Student figure cannot be fetched; it lives in the documented table. Pro's
    // allowance IS on the page and resolves to $5 — pinned separately in
    // `zed-allowance.spec.ts`.
    const usage = parseZedUsage({
      plan: {
        plan_v3: 'zed_student',
    ...
    assert.equal(usage.windows?.length, 1)
    assert.equal(usage.windows?.[0]?.scope, 'Hosted models')
    assert.equal(usage.windows?.[0]?.limit, 10, 'the Student bundle is a published product fact, not a guess')
